        ...
        <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <select name="category" id="category" required
                    ...
                    <option value="Wearables" {{ old('category', $product->category) == 'Wearables' ? 'selected' : '' }}>Wearables</option>
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="4"
                    class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white @error('description') border-red-500 @enderror">{{ old('description', $product->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            @if($product->image)
                <div class="mb-4">
                    ...
                    <div class="flex items-center gap-4">
                        <img src="{{ Storage::url($product->image) }}" alt="{{ $product->name }}" class="w-32 h-32 object-cover rounded-lg border border-gray-300">
                        <div>
                            <p class="text-sm text-gray-600 dark:text-gray-400">Upload new image to replace</p>
                        </div>
                    </div>
                </div>
            @endif
